      ...
      >
        <div className="flex items-center justify-between px-5 h-16 border-b border-[var(--color-border)] shrink-0">
          <h2 className="text-lg font-bold text-[var(--color-text)] font-display m-0 flex items-center gap-2">
            ...
            My Favourites
            {favourites.length > 0 && (
              <span className="text-[0.8rem] font-normal text-[var(--color-text-muted)]">({favourites.length} {favourites.length === 1 ? 'item' : 'items'})</span>
            )}
          </h2>
          <button type="button" className="w-10 h-10 rounded-full border-none bg-[var(--color-border)] text-[var(--color-text)] cursor-pointer inline-flex items-center justify-center text-base transition-[background,color] duration-180 hover:bg-[var(--color-button)] hover:text-white" onClick={onClose} aria-label="Close favourites">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {favourites.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-[var(--color-text-muted)] gap-3">
              <i className="fa-regular fa-heart text-5xl" aria-hidden="true" />
              ...
              {favourites.map((item) => (
                <li key={item.productId} className="flex gap-3 p-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)]">
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-between gap-1.5">
                    ...
                    <span className="text-[1rem] font-bold text-[var(--color-button)]">${item.product.price.toFixed(2)}</span>
                    <div className="flex items-center justify-between gap-2">
                      <button
                        type="button"
                        className={`text-[0.8rem] font-semibold px-3 py-1.5 rounded-full border-none cursor-pointer transition-[background,color] duration-150 ${
                      ...
                      <button type="button" className="w-7 h-7 rounded-full border-none bg-transparent text-[var(--color-text-muted)] cursor-pointer inline-flex items-center justify-center text-[0.85rem] transition-[color] duration-150 hover:text-[var(--color-danger)]" onClick={() => removeFromFavourites(item.productId)} aria-label="Remove from favourites">
